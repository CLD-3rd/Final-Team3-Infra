...
resource "aws_iam_role_policy_attachment" "ca_policy_attach_ec2readonly" {
  role       = aws_iam_role.ca_irsa.name
...
# ASG 태그에 eks.amazonaws.com/nodegroup 값 넘겨주는 부분
data "aws_autoscaling_groups" "nodegroup_asgs" {
  filter {
...
resource "aws_autoscaling_group_tag" "ca_nodegroup_tag" {
  autoscaling_group_name = data.aws_autoscaling_groups.nodegroup_asgs.names[0]
  tag {
