...
resource "aws_subnet" "public" {
  ...
  cidr_block              = var.public_subnet_cidr             # 퍼블릭 서브넷 CIDR (예: 10.0.1.0/24)
  map_public_ip_on_launch = true                               # 인스턴스 시작 시 자동 퍼블릭 IP 할당
  availability_zone       = var.az                             # 배치할 가용 영역 (예: ap-northeast-2a)
...
resource "aws_subnet" "private" {
  vpc_id            = aws_vpc.this.id                          # 연결할 VPC 지정
  cidr_block        = var.private_subnet_cidr                  # 프라이빗 서브넷 CIDR (예: 10.0.2.0/24)
  availability_zone = var.az                                   # 배치할 가용 영역
...
resource "aws_route_table_association" "public_assoc" {
  subnet_id      = aws_subnet.public.id                        # 연결할 퍼블릭 서브넷
